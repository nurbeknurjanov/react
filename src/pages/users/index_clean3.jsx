...
let UsersView = ({users, getUsersOperation, history, location})=>{
    ...
    //constructor
    let filter = getQuery().get('filter');
    ...
    const submitForm = ()=>{
        history.push({
            pathname:'/users',
            search: new URLSearchParams({filter:filterRef.current.value,page:1}).toString()
        })
    }
    ...
    return <div>
        ...
        <input ref={filterRef} defaultValue={filter} onKeyPress={setFilterHandler} placeholder='Filter'/>
        <br/>
        <ul>
            {users.map((el, index)=>{
                return <li key={index}>
